            ...
            <div className="grid gap-8 md:grid-cols-[1fr,2fr]">
                ...
                <div className="space-y-6">
                    <div className="p-8 border border-zinc-800 rounded-xl bg-card flex flex-col items-center text-center">
                        ...
                        <h2 className="text-xl font-medium text-white mb-1">{customer.name}</h2>
                    ...
                    <div className="p-6 border border-zinc-800 rounded-xl bg-card space-y-4">
                        <h3 className="font-medium text-white border-b border-zinc-800 pb-2">Contact Info</h3>
                        <div className="space-y-3 text-sm">
                            <div>
                                <span className="text-zinc-500 block mb-1">Phone</span>
                                <span className="text-zinc-300">{customer.phone || "Not provided"}</span>
                            </div>
                            <div>
                                <span className="text-zinc-500 block mb-1">Address</span>
                                <span className="text-zinc-300 whitespace-pre-line">{customer.address || "No address"}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="p-4 border border-zinc-800 rounded-xl bg-card">
                            <span className="text-xs text-zinc-500 uppercase tracking-wide">Total Orders</span>
                            <p className="text-2xl font-bold text-white mt-1">{customer.stats.totalOrders}</p>
                        </div>
                        <div className="p-4 border border-zinc-800 rounded-xl bg-card">
                            <span className="text-xs text-zinc-500 uppercase tracking-wide">Total Spent</span>
                            <p className="text-2xl font-bold text-white mt-1">${customer.stats.totalSpent.toFixed(2)}</p>
                        </div>
                        <div className="p-4 border border-zinc-800 rounded-xl bg-card">
                            <span className="text-xs text-zinc-500 uppercase tracking-wide">Avg. Order</span>
                            <p className="text-2xl font-bold text-white mt-1">${customer.stats.avgOrderValue.toFixed(2)}</p>
                        </div>
                    </div>
                    ...
                    <div className="border border-zinc-800 rounded-xl bg-card overflow-hidden">
                        ...
                        <div className="divide-y divide-zinc-800">
                            {customer.recentOrders.map((order) => (
                                <div key={order.id} className="p-4 flex items-center justify-between hover:bg-zinc-900/50 transition-colors">
                                    <div className="flex flex-col gap-1">
                                        <Link href={`/orders/${order.id}`} className="font-medium text-sm text-blue-400 hover:underline">
                                            #{order.id}
                                        </Link>
                                        <span className="text-xs text-zinc-500">{new Date(order.date).toLocaleDateString()}</span>
                                    </div>
                                    <div className="flex items-center gap-6">
                                        ...
                                        <span className="text-sm font-medium w-16 text-right">${order.total}</span>
                                    </div>
                                </div>
                            ))}
