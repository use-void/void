...
export interface UsePaymentReturn {
  isLoading: boolean;
  error: string | null;
  pay: (provider: 'moyasar' | 'stripe', options: CreatePaymentOptions) => Promise<PaymentResult>;
}
...
export function usePayment(): UsePaymentReturn {
  ...
  const pay = async (provider: 'moyasar' | 'stripe', options: CreatePaymentOptions): Promise<PaymentResult> => {
    setIsLoading(true);
    setError(null);
    try {
